...
app.use(cookieParser("abdi349scnsndc983nf"));
app.use("/index", indexRouter);
app.use(upload.single("image"));
app.use(session({
    resave:false,
    saveUninitialized:false,
    secret:"abdi349scnsndc983nf",
    cookie:{
        httpOnly:true,
    }
}));
...
app.get("/", (req, res) => {
    console.log("cookie", decodeURIComponent(req.cookies['user']));
    console.log(req.cookies)
    console.log(req.session)
    console.log(req.session.userid)
    if (req.cookies['user']) {
        res.render("form", {"user": decodeURIComponent(req.cookies['user'])});
    } else {
        res.render("form");
    }
});
...
app.post("/", (req, res) => {
    ...
    res.cookie("user", encodeURIComponent(userName), {
        httpOnly: true,
        path: "/",
        maxAge:5000
    });
